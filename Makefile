...
# --- Directories ---
BUILD_DIR = build
LIB_ROOT_DIR = externals/hoshi/Lib
INC_DIR = externals/hoshi/include
TOOL_DIR = externals/hoshi/packtool
OUT_DIR = out
...
MOD_ALL_SOURCES := $(foreach mod,$(MOD_NAMES),\
...
MOD_OBJECTS := $(patsubst $(MODS_ROOT_DIR)/%.c,$(BUILD_DIR)/$(MODS_ROOT_DIR)/%.o,$(MOD_ALL_SOURCES))

# Combine ALL individual object files (from both libraries and mods) that need to be compiled.
ALL_INDIVIDUAL_OBJECTS_TO_COMPILE = $(LIB_OBJECTS) $(MOD_OBJECTS)
...
# Each $(OUT_DIR)/<mod_name>.bin depends on its specific linked mod object and the output directory.
$(OUT_DIR)/%.bin: $(BUILD_DIR)/%.modlink $(OUT_DIR)
	@echo ""
